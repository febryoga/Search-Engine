...
function hitungsim($query) {
	...
	for ($i=0; $i<count($aquery); $i++) {
		...
		$resNTerm = mysqli_query($koneksi,"SELECT Count(*) as N from tbindex WHERE Term = '$aquery[$i]'");
		$rowNTerm = mysqli_fetch_array($resNTerm);
		$NTerm = $rowNTerm['N'] ;
    //$idf = 0;
    $idf = 0;
	...
	while ($rowDocId = mysqli_fetch_array($resDocId)) {
		...
		$resTerm = mysqli_query($koneksi,"SELECT * FROM tbindex WHERE DocId = $docId");
		...
		if ($dotproduct > 0) {
			...
			$resInsertCache = mysqli_query($koneksi,"INSERT INTO tbcache (Query, DocId, Value) VALUES ('$query', $docId, $sim)");
			$jumlahmirip++;
	...
	if ($jumlahmirip == 0) {
		$resInsertCache = mysqli_query($koneksi,"INSERT INTO tbcache (Query, DocId, Value) VALUES ('$query', 0, 0)");
	}
...
function ambilcache($query) {
	include "fungsi/koneksi.php";
	$resCache = mysqli_query($koneksi,"SELECT *  FROM tbcache WHERE Query = '$query' group by DocId ORDER BY Value DESC LIMIT 5");
	...
	if ($num_rows >0) {
		...
		$resCache = mysqli_query($koneksi,"SELECT *  FROM tbcache WHERE Query = '$query ORDER BY Value DESC");
		$num_rows = mysqli_num_rows($resCache);
